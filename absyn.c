...
A_var
A_FieldVar(A_pos pos, A_var var, S_symbol sym)
{
  A_var p = checked_malloc(sizeof(*p));
  p->kind = A_fieldVar;
  p->pos = pos;
...
A_var
A_SubscriptVar(A_pos pos, A_var var, A_exp exp)
{
  A_var p = checked_malloc(sizeof(*p));
...
A_exp
A_StringExp(A_pos pos, string s)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_stringExp;
  p->pos = pos;
...
A_exp
A_OpExp(A_pos pos, A_oper oper, A_exp left, A_exp right)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_opExp;
  p->pos = pos;
...
A_exp
A_RecordExp(A_pos pos, S_symbol typ, A_efieldList fields)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_recordExp;
  p->pos = pos;
...
A_exp
A_SeqExp(A_pos pos, A_expList seq)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_seqExp;
  p->pos = pos;
...
A_exp
A_AssignExp(A_pos pos, A_var var, A_exp exp)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_assignExp;
  p->pos = pos;
...
A_exp
A_IfExp(A_pos pos, A_exp test, A_exp then, A_exp elsee)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_ifExp;
  p->pos = pos;
...
A_exp
A_WhileExp(A_pos pos, A_exp test, A_exp body)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_whileExp;
  p->pos = pos;
...
A_exp
A_ForExp(A_pos pos, S_symbol var, A_exp lo, A_exp hi, A_exp body)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_forExp;
  p->pos = pos;
...
A_exp
A_LetExp(A_pos pos, A_decList decs, A_exp body)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_letExp;
  p->pos = pos;
...
A_exp
A_ArrayExp(A_pos pos, S_symbol typ, A_exp size, A_exp init)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind = A_arrayExp;
  p->pos = pos;
...
A_dec
A_FunctionDec(A_pos pos, A_fundecList function)
{
...
A_dec
A_VarDec(A_pos pos, S_symbol var, S_symbol typ, A_exp init)
{
  A_dec p = checked_malloc(sizeof(*p));
  p->kind = A_varDec;
  p->pos = pos;
...
A_dec
A_TypeDec(A_pos pos, A_nametyList type)
{
  A_dec p = checked_malloc(sizeof(*p));
  p->kind = A_typeDec;
  p->pos = pos;
...
A_ty
A_NameTy(A_pos pos, S_symbol name)
{
...
A_ty
A_RecordTy(A_pos pos, A_fieldList record)
{
  A_ty p = checked_malloc(sizeof(*p));
  p->kind = A_recordTy;
  p->pos = pos;
...
A_ty
A_ArrayTy(A_pos pos, S_symbol array)
{
  A_ty p = checked_malloc(sizeof(*p));
  p->kind = A_arrayTy;
  p->pos = pos;
...
A_field
A_Field(A_pos pos, S_symbol name, S_symbol typ)
{
...
A_fieldList
A_FieldList(A_field head, A_fieldList tail)
{
...
A_fundec
A_Fundec(A_pos pos, S_symbol name, A_fieldList params, S_symbol result,
         A_exp body)
{
...
A_fundecList
A_FundecList(A_fundec head, A_fundecList tail)
{
...
A_decList
A_DecList(A_dec head, A_decList tail)
{
...
A_namety
A_Namety(S_symbol name, A_ty ty)
{
...
A_nametyList
A_NametyList(A_namety head, A_nametyList tail)
{
...
A_efield
A_Efield(S_symbol name, A_exp exp)
{
...
A_efieldList
A_EfieldList(A_efield head, A_efieldList tail)
{
  ...
  A_exp translated = A_LetExp(
    ...
    A_WhileExp(
      ...
      A_SeqExp(0, A_ExpList(a->u.forr.body,
                            A_ExpList(A_AssignExp(0, cntvar,
                                                  A_OpExp(0, A_plusOp, cntexp,
                                                          A_IntExp(0, 1))),
