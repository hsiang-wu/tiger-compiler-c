...
SEM_transProg(A_exp exp)
{
  ...
  struct expty et =
    transExp(venv, tenv, exp, Tr_newLevel(out, S_Symbol("main"), NULL),
             // out,
             NULL);
...
Ty_tyList
makeFormalTyList(S_table tenv, A_fieldList afl)
{
...
struct expty
transExp(S_table venv, S_table tenv, A_exp a, Tr_level level, Tr_exp brk)
{
  switch (a->kind) {
    case A_varExp: {
    ...
    case A_recordExp: {
      ...
      Tr_expList el_reversed = checkRecordFieldList(
        tenv, venv, a->u.record.fields, recordty->u.record, level, brk);
    ...
    case A_seqExp: {
      ...
      if (el == NULL) return expTy(Tr_nilExp(), Ty_Void());
    ...
    case A_assignExp: {
      ...
      if (var.ty != exp.ty)
        EM_error(a->pos, "unmatched assign exp %d %d", var.ty->kind,
                 exp.ty->kind);
    ...
    case A_ifExp: {
      dprintf(" if!\n");
      struct expty cond;
      if ((cond = transExp(venv, tenv, a->u.iff.test, level, brk)).ty !=
          Ty_Int())
    ...
    case A_whileExp: {
      ...
      if ((test = transExp(venv, tenv, a->u.whilee.test, level, brk)).ty !=
          Ty_Int())
    ...
    case A_arrayExp: {
      ...
      if (size.ty != Ty_Int()) EM_error(a->pos, "Array length must be int.");
...
struct expty
transVar(S_table venv, S_table tenv, A_var v, Tr_level level, Tr_exp brk)
{
  switch (v->kind) {
    case A_simpleVar: {
    ...
    case A_fieldVar: {
      ...
      if (et.ty->kind == Ty_record) {
        ...
        // XXX:ugly code here to get the length of the list
        Ty_fieldList l_;
        for (l_ = et.ty->u.record; l_; l_ = l_->tail, len++) {
        }
        ...
        if (i < len)
          return expTy(Tr_fieldVar(et.exp, len - i - 1), tmpty);
        else
...
Tr_exp
transDec(S_table venv, S_table tenv, A_dec d, Tr_level level, Tr_exp brk)
{
  switch (d->kind) {
    case A_varDec: {
    ...
    case A_functionDec: {
      ...
      for (fl = d->u.function; fl; fl = fl->tail) {
        ...
        for (Ty_tyList t = formalTys; t; t = t->tail) {
          escapeList = U_BoolList(TRUE, escapeList); // always escape, XXX:
                                                     // escapeList is reversed
                                                     // compared with tylist
        }
        ...
        S_beginScope(venv);
        {
          ...
          Ty_tyList t;
          Tr_accessList accs =
            AccTail(Tr_formals(newlev)); // head is static link
          // XXX: reconsider the reverse/sequence of parameters.
          for (l = f->params, t = formalTys; l;
               l = l->tail, t = t->tail, accs = AccTail(accs)) {
            ...
            S_enter(venv, l->head->name, E_VarEntry(AccHead(accs), t->head));
          }
        }
...
Ty_fieldList
makeTyFieldList(S_table tenv, A_fieldList afl)
{
...
Tr_expList
checkRecordFieldList(S_table tenv, S_table venv, A_efieldList efl,
                     Ty_fieldList fl, Tr_level level, Tr_exp brk)
{
  Tr_expList el = NULL;
...
Ty_tyList
nametyList(S_table tenv, A_nametyList nl)
{
...
void
checkTypeDec(S_table tenv, Ty_tyList tl)
{
...
void
checkRecord(S_table tenv, Ty_fieldList fl)
{
